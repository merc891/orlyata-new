...
| --- | --- | --- | --- |
| `Button` `4651:1140` | `template-parts/components/button.php` — реализован | primary/secondary, play, arrows, gallery arrows (Neutral 50 → Neutral 150 hover), `menu-tablet`; default/hover/active/disabled/loading в витрине, реальный keyboard focus-visible; `menu-tablet` принадлежит Sidebar на 768–1279 px, сохраняет 48 px target и morph-ится из Morphicons Menu в X (24 px, stroke 2) вместе с `aria-expanded` | отдельная mobile-настройка — этап 6 |
...
| `Card` `4651:1333` | `media-card.php` — реализован | Big сохраняет ratio `797/540`, Small — `390/540`; image region Small сохраняет desktop-долю `300/540` (`5/9` высоты); first Badge is date; second is provider only for video; inverse Badge у big; static image; content/Play above gradient; Figma Play at 24/24 Big and 24/16 Small; оболочка, image region, title measure, padding, Badge и внутренние gaps масштабируются вместе. Для Home tablet первых трёх секций утверждено только page-scoped размещение: Big full-row, Small в две колонки; desktop-owned ratios `797/540` и `390/540` и внутренности сохраняются, а mobile их не наследует. Home вправе менять только одобренный span, порядок и потоковое размещение; иной ratio сначала оформляется named variant компонента | photo/video, missing image, focus, long title, responsive visual/geometry checks |
...
| `Sidebar` `4641:3718` | `layout/sidebar.php` — реализован | reference 1920 px: 250 px; tablet `768–1279`: shared top header uses `logo-tablet.svg`, `11.25rem` × `6.25625rem` (180 × 100.1 px at 1279) and `menu-tablet`; its menu panel slides down over 400 ms easing-out, then links and CTA fade in over 400 ms. Links use Heading 2 with no left padding and start 48 px below the logo; mobile retains the existing shared header logo at `4.68rem` high; default/hover/focus-visible/active/current-page; semantic primary navigation and admission CTA | JavaScript only enhances the server-rendered controls; navigation and CTA remain available without it |
| `Footer` `4641:3638` | `layout/footer.php` — реализован | reference 1920 px: 1616×469 px; tablet 768–1279 px: natural-height two-column flow; mobile 320–767 px: natural-height single-column flow; default/hover/focus-visible/active; navigation, dynamic contacts, privacy link | shared layout only; no page-specific Footer variants |

...
| --- | --- | --- | --- |
| `Button` `4651:1140` | `template-parts/components/button.php` — реализован | primary/secondary, play, arrows; default/hover/active/disabled/loading в витрине, реальный keyboard focus-visible; Primary opacity и Secondary/Arrow surface hover за 200ms | отдельная tablet/mobile настройка — этап 6 |
...
| `Link` `4651:1238` | `text-link.php` — реализован | roll, color, color-inverse; default/hover/active | focus-visible, visited policy |
...
| `Table_row` + `Table_header` | `data-table.php` — реализован | achievements; news/photo/video: Name spans desktop editorial columns 1–2, Date starts at 3, Type and right-aligned accessible arrow link occupy 4; cursor pointer; hover/focus-within turns all row content secondary and rolls the arrow left-to-right; video replaces the arrow with a centered 32px provider icon (YouTube, RuTube or VK) that remains static on hover; 1px нижняя граница каждой data-строки | семантические таблицы, mobile mapping, empty state, pagination |
...
- News list: below the filters by `--news-filter-table-gap` (32 px at 1920), page composition reuses DataTable `news`; its desktop four-column allocation matches the photo/video DataTable variant. The server and client share its row-category hidden state, so multi-select filters work after a reload and without one.
- Media preview;
- History;
- Choir groups;
- Teachers;
